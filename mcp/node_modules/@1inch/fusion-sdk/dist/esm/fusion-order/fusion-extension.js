...
export class FusionExtension {
    ...
    static fromExtension(extension) {
        ...
        const fees = new FeeTakerExt.Fees(interactionData.fees.resolverFee.isZero()
            ? FeeTakerExt.ResolverFee.ZERO
            : new FeeTakerExt.ResolverFee(protocolFeeRecipient, interactionData.fees.resolverFee, interactionData.fees.whitelistDiscount), interactionData.fees.integratorFee.isZero()
            ? FeeTakerExt.IntegratorFee.ZERO
            : new FeeTakerExt.IntegratorFee(integratorFeeRecipient, protocolFeeRecipient, interactionData.fees.integratorFee, interactionData.fees.integratorShare));
        return new FusionExtension(settlementContract, auctionDetails, whitelist, {
            makerPermit,
            fees,
            customReceiver
    ...
    buildAmountGetterData(forAmountGetters) {
        ...
        const integrator = {
            fee: this.extra?.fees?.integrator.fee.toFraction(FeeTakerExt.Fees.BASE_1E5) || 0,
            share: this.extra?.fees?.integrator.share.toFraction(FeeTakerExt.Fees.BASE_1E2) || 0
        };
        const resolverFee = this.extra?.fees?.resolver.fee.toFraction(FeeTakerExt.Fees.BASE_1E5) || 0;
        ...
        builder
            ...
            .addUint8(BigInt(Number(FeeTakerExt.Fees.BASE_1E2) -
    ...
    getFeesForTaker(taker) {
        const whitelistDiscount = this.extra?.fees?.resolver.whitelistDiscount.toFraction(FeeTakerExt.Fees.BASE_1E2) || 0;
        ...
        const integratorFeeBN = BigInt(this.extra?.fees?.integrator.fee.toFraction(FeeTakerExt.Fees.BASE_1E5) || 0);
        return {
    ...
    getTakingAmountWithFee(taker, orderTakingAmount) {
        ...
        return mulDiv(orderTakingAmount, FeeTakerExt.Fees.BASE_1E5 + fees.resolverFee + fees.integratorFee, FeeTakerExt.Fees.BASE_1E5, Rounding.Ceil);
    }
}
FusionExtension.CUSTOM_RECEIVER_FLAG_BIT = 0n;
function parseAmountData(iter) {
    const fees = {
        integratorFee: Bps.fromFraction(Number(iter.nextUint16()), FeeTakerExt.Fees.BASE_1E5),
        integratorShare: Bps.fromFraction(Number(iter.nextUint8()), FeeTakerExt.Fees.BASE_1E2),
        resolverFee: Bps.fromFraction(Number(iter.nextUint16()), FeeTakerExt.Fees.BASE_1E5),
        whitelistDiscount: Bps.fromFraction(Number(FeeTakerExt.Fees.BASE_1E2) - Number(iter.nextUint8()), FeeTakerExt.Fees.BASE_1E2)
    };
    return {
        fees
    };
